...
extension CGImageSource {
     ...
     */
    // GImageSourceCopyAuxiliaryDataInfoAtIndex関数からAuxiliaryデータを取得
    // 第一引数はCGImageSourceオブジェクト(ここにデプスデータが入ってる)
    // 第三引数には取り出したいAuxiliary(補助)データのタイプを指定，ここではDisparityになっている
    // 返却値はCFDictionary型オブジェクトが存在しなければnilが返ってくる
    // デプスマップのCFDataRefやメタデータのCGImageMetadataRefが入っている
    // この次はAVDepthDataを初期化する?
    ...
    // fromDictionaryRepresentationを取得する(このデータにはプスデータが入っている)
